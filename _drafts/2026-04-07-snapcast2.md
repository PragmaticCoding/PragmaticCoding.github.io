...
This is the third part of an article describing how to set up whole-home audio using Mopidy and SnapCast.  If you haven't already read them should should probably read parts I and II:




In this article we'll look at how to add Mopidy as a service to your SnapCast server.
...
# python3 -m pip install --break-system-packages Mopidy-Muse
# python3 -m pip install --break-system-packages Mopidy-Jellyfin
```
...
``` bash
128  apt install gstreamer1.0-plugins-good
130  apt install gstreamer1.0-plugins-bad
131  apt install gstreamer1.0-plugins-ugly
```
...
The `output` tag is every important.  Make sure that the "filesink location" matches the fifo pipe that you created for SnapCast.  If you followed these instructions, it should be `/var/local/snapserver/snapfifo`.
...
In our house, we want the whole home audio streaming SomaFM's "GrooveSalad" from the time we wake up until we go to bed.  When our current dog was just a puppy, and we had her spending the night in a crate beside the bed, we needed some background noise so that she wouldn't wake up and start whining whenever someone rolled over or made some small sound.  So we started using Google to play "Forest Sounds" at night.  Over time, we became used to this, and found it filters out street noises and other nighttime distractions.  

With this new setup, I was looking for a way to have a completely automated, "hands-free", solution that would turn on GrooveSalad in the morning, and turn on some kind of forest sounds at night.  Something that just worked, and that I didn't have to even think about.
...
In order to investigate how to use the API, I downloaded an application called, "Bruno", which is an open source alternative to "Postman".  This application allows you to create REST API calls that you can test and see the results in real time.
...
How to run these? Well, `cron` of course!
...
```
I've left in the default comments.  I need this to run as `root`, so `sudo` is used here. The command `crontab -l` just lists the crontab file.  These two commands run every day, the first one at 7:30 AM, and the second at 9:00 PM.
...
```
Eventually, I'll remove the `echo` commands, since they really aren't needed.  In my running version, I have a line that submits a message to my `Gotify` server in the event that it detects that the stream has stopped.
...
Success in these kinds of projects is always evaluated in how happy my wife is with how it works.  She was getting quite fed up with the Google issues that we were experiencing, and frustrated with the constant drops in the service and times when it simply wouldn't start up again.  
...
I goofed when I implemented the automated stream recovery system from the previous section.  

I had tested my script manually and it worked perfectly, but once I installed it into `crontab` we were getting constant glitches in the audio feed.  Curiously, it seemed to be every 3 minutes.  These were really short disruptions in the audio, generally less than a second.

It turns out that I had forgotten to put `#!/bin/bash` at the top of my script.  When I tested it manually, it used `bash` because that's the shell that I use.  But `crontab` uses `sh`.  Lots of stuff works just the same between `sh` and `bash`, but some of the slightly more advanced stuff in `bash` doesn't work in `sh`.  Consequently, my date/time comparison in the `if` condition was always failing.  This meant that it restarted the stream every 3 seconds.

It's embarassing how long it took me to figure that out.  
